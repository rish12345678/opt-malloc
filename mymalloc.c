...
void *mymalloc(size_t s) {
  ...
  size_t small_block_limit = page_size - sizeof(block_t);

  if (s > small_block_limit) {
    size_t needed = s + sizeof(block_t);
  ...
  while (curr) {
    if (curr->size >= s) {
      ...
      if (curr->size >= s + sizeof(block_t) + sizeof(block_t)) {
        block_t *new_block = (block_t*)((char*)curr + sizeof(block_t) + s);
        new_block->size = curr->size - s - sizeof(block_t);
  ...
  block->size = page_size - sizeof(block_t);

  if (block->size >= s + sizeof(block_t) + sizeof(block_t)) {
